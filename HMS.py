...
class HMSProcessor(object):
    ...
    def __init__(self, image, lcmr_m, k, m, a_1, a_2, mc):
        ...
        # 改后
        self.GridStep = np.array(self.GridStep).astype(np.intp)
    ...
    def init_cluster(self):
        ...
        number_of_clusters = (int((self.Height - int(self.GridStep / 2)) / (self.GridStep) + 1)) * (
            int((self.Width - int(self.GridStep / 2)) / (self.GridStep) + 1))
    ...
    def splitting_cells(self):
        ...
        for c_i in range(self.ClusterNumber):
            ...
            if (self.iter > 0 and self.Clusters[c_i, 3 + self.Bands] < self.splitting_threshold and self.Clusters[
                c_i, 2 + self.Bands] > self.local_search_range / 4):
                split_count += 1
                split_status[c_i] = 1
    ...
    def merging(self, c_i, label):
        ...
        new_cluster = self.Clusters[c_i, :] * self.Clusters[c_i, (2 + self.Bands)] + self.Clusters[label, :] * \
                      self.Clusters[label, (2 + self.Bands)]
    ...
    def merging_cells(self):
        ...
        # 改后
        self.visited_status = -1 * np.ones((self.ClusterNumber), dtype=np.intp)
        ...
        for c_i in range(self.ClusterNumber):
            ...
            if (self.visited_status[c_i] == -1):
                ...
                [neighbours, count] = HMSCython.neighbour_search(c_i_positions, self.Labels, self.visited_status,
                                                                 self.GridStep)
                ...
                if (neighbours and self.Clusters[c_i, (2 + self.Bands)] < self.local_search_range / 32):
                    ...
                    for i in range(len(neighbours)):
                        ...
                        neighbour_differences[i] = np.linalg.norm(
                            self.Clusters[c_i, 2:(2 + self.Bands)] - self.Clusters[neighbours[i], 2:(2 + self.Bands)])
                    ...
                    for i in range(len(neighbours)):
                        combined_areas[i] = (self.Clusters[c_i, (2 + self.Bands)] + self.Clusters[
                            neighbours[i], (2 + self.Bands)]) / (self.local_search_range / 5)
        ...
        new_lcmr = np.zeros(
            (self.ClustersLCMR.shape[0], self.ClustersLCMR.shape[0], self.ClusterNumber - merging_count))
    ...
    def assignmentCython(self):
        ...
        # 改后
        clusterPositions = np.ascontiguousarray(self.Clusters[:, 0:2], dtype=np.intp)
        ...
        gridStep = np.array(gridStep).astype(np.intp)

        compactness = float(self.Compactness)

        self.Labels, self.Distances = HMSCython.assign_clusters_cython(imageGrid, gridStep, cluster_number,
                                                                       distanceArray, clusterPositions, clusterColours,
                                                                       clusterLCMR, clusterScales, compactness,
                                                                       self.lcmr, self.a_1, self.a_2)
        ...
        # 现版
        self.Labels = self.Labels.astype(np.intp)
    ...
    def updateClustersCython(self):
        ...
        if (self.Clusters[0, 0] > 0):
            ...
            [kvector_info, kvector_lcmr] = np.asarray(
                HMSCython.update_cluster_cython(self.ImageArray, self.lcmr, self.Labels, self.ClusterNumber,
                                                self.Areas))
            # Updates the position colour and area but leaves the scaling factor the same
            for i in range(self.ClusterNumber):
                if (kvector_info[0, i] != 10000.0):
                    error = np.linalg.norm(
                        self.Clusters[i, :(2 + self.Bands)] - kvector_info[:(2 + self.Bands), i]) ** 2
                ...
                if (kvector_info[0, i] == 10000.0):
                    ...
                    self.Clusters[i, 2:(2 + self.Bands)] = self.ImageArray[int(self.Clusters[i, 0]),
                                                           int(self.Clusters[i, 1]), :]
        ...
        # Prevent residual error being calculated for the possible update after connectivity
            ...
            # The updated cluster information is stored in the kvector_info. In the form h,w,L,a,b
            kvector_info, kvector_lcmr = HMSCython.update_cluster_cython(self.ImageArray, self.lcmr, self.Labels,
                                                                         self.ClusterNumber, self.Areas)
    ...
    def enforceConnectivity(self):
        ...
        min_size = np.array(min_size).astype(np.intp)
        max_size = np.array(max_size).astype(np.intp)
