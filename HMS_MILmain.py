...
from SupLevel import superpixelExtract, Sup2Mi, bagid_convert, milab2segmap_sup, segmap2acc, wrappedSup, supResFusion
from PixLevel import pixco2pixbag, pixres2resmap, resmapFusion, spatCer
from bagTranform import bagEmbed, miVLAD, miFAFV
from sklearn.svm import SVC
from sklearn.decomposition import PCA, NMF
from bagGraph import bagGraph, localbagGraph
from OVOClassifier import ovoBRSVM, ovoSVM
...
hyper_data_f = np.array(hyper_data, dtype='float32')
hyper_data_d = np.array(hyper_data, dtype='double')

# nmf = NMF(n_components=20,  init='warn')
# nmf_hyper = nmf.fit_transform(np.reshape(hyper_data_d, newshape=(-1, band)))
# hyper_data_d = np.reshape(nmf_hyper, newshape=(length, width, -1))
...
for i in range(repeat):
    tt1 = time.time()
    res_map, supcermap, tr_pixcos, te_pixcos, rest_pixcos, inf_t = wrappedSup(hyper_data_d, supload, maindir, K[0], gt_img, gt_n,
                                                                       tr_ratio, C, gamma, length, width, n1, n2,
                                                                       method, seed[i], dataset)

    res_map1, supcermap1, tr_pixcos1, te_pixcos1, rest_pixcos1, inf_t1 = wrappedSup(hyper_data_d, supload, maindir, K[1], gt_img,
                                                                            gt_n, tr_ratio, C, gamma,
                                                                            length, width, n1, n2, method, seed[i], dataset)

    res_map2, supcermap2, tr_pixcos2, te_pixcos2, rest_pixcos2, inf_t2 = wrappedSup(hyper_data_d, supload, maindir, K[2],
                                                                            gt_img, gt_n, tr_ratio, C, gamma, length,
                                                                            width, n1, n2, method, seed[i], dataset)
    tt2 = time.time()
    t1 = tt2 - tt1
