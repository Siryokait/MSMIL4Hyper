...
def dimensional_reduction_mnf(spectral_data, number_of_components):
    ...
    ## Centre the data
    centre_data = spectral_data_2d - np.ones((height * width, 1)) * np.reshape(np.average(spectral_data_2d, axis=0),
                                                                               (1, bands))
...
def create_logm_matrices(spectral_mnf, window_size, K):
    ...
    for i in range(size_0):
        ...
        for j in range(size_1):
            ...
            M_matrix = np.maximum(np.transpose(np.matlib.repmat(M - m, d, 1)), 1e-8)
