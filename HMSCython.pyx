...
def assign_clusters_cython(double[:, :, ::1] imageGrid, Py_ssize_t gridStep, Py_ssize_t cluster_number,
    ...
    # 更改后代码
    cdef Py_ssize_t[:,::1] labels = -1*np.ones((height,width),dtype = np.intp)
...
def where_cluster(Py_ssize_t[: , ::1] labels,Py_ssize_t cluster_number):
    ...
    # 改后
    cdef Py_ssize_t[::1] count = np.zeros((cluster_number),dtype=np.intp)
    cdef Py_ssize_t[:,:,::1] positions = np.zeros((cluster_number,segement_size,2),dtype = np.intp)
...
def neighbour_search(Py_ssize_t[:,::1] indexs,Py_ssize_t[:,::1] labels,Py_ssize_t[::1] visited,Py_ssize_t S):
    ...
    # 改后
    cdef Py_ssize_t[::1] dx = np.array((1 , -1 , 0 , 0),dtype = np.intp)
    cdef Py_ssize_t[::1] dy = np.array((0 , 0 , 1 , -1),dtype = np.intp)
    ...
    # 改后
    cdef Py_ssize_t[::1] neighbours = -1*np.ones((2*height + 2*width),dtype = np.intp)
